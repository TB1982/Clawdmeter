...
Weather comes from [Open-Meteo](https://open-meteo.com) — no account, no key — at most once every ten minutes. The moon icon appears only while the moon is actually above the horizon, which is not the same thing as night. Quotes come from the Taiwan exchange's own endpoint and are labelled *at last close* outside 09:00–13:30 on weekdays, because for most of any day the number is not live and an unlabelled price is a quiet lie.
